	...
	<view class="main-wrap">
			...
			<template v-for="(item, index) in list">
				<view class="table-row" :key="index" @click="viewDetail(item)">
					...
					<view class="table-box actualState"
						:class="{'green': item.actualState === 2, 'red': item.actualState === 3,'black': item.actualState === 4}">
						{{actualStateEnum[item.actualState]}}
						<view v-if="item.actualState === 1 && !readOnly" class="del-btn"
							@click.stop="delListMore(index)">
		...
		<u-overlay :show="this.isShowTips" @click="isShowTips = false">
			<view class="tips-block">
				<view class="tips-title">未盘剩余 {{ changeCount}} 请检查!</view>
	...
	export default {
		...
		methods: {
				...
				if (!this.alreadySet.has(str)) {
					...
					this.list.forEach(res => {
						...
						if (str == res.assetsEpc) {
								if(res.actualState==0 || res.actualState==4){
									res.actualState = 2
									this.alreadyCount++
			...
			handleSave() {
				...
				uni.request({
					url: 'http://124.222.61.241:6868/api/ESC/App_InventorySave_Eam',
					method: 'POST',
					data: {
			...
			reset() {
              ...
              this.alreadySet.clear()
				...
				this.alreadyCount=0
							this.surplusCount = 0
							uni.request({
								url: 'http://124.222.61.241:6868/api/ESC/App_InventoryPageAssetsList_Eam',
								method: 'POST',
								data: {
								...
								success: (res) => {
									...
									if (res.data.code != 0) {
										...
										this.list.forEach(item => {
											...
											if ([0, 1, 2,4].includes(item.actualState)) {
												this.totalCount += 1
											}
											// 维护已盘集合
											...
											//未盘
											if (item.actualState === 0||item.actualState === 4) {
												this.changeCount += 1
											}
										})
									}
					...
					success: (res) => {
						...
						if (res.data.code != 0) {
							...
							this.list.forEach(item => {
								...
								// 维护已盘集合
								if ([1, 2, 3].includes(item.actualState)) {
									// this.alreadySet.add(item.Assets_Epc)
								}
								// 已盘点计数，包含已盘和变更
								if (item.actualState === 2) {
			...
			confirmApproval() {
				uni.request({
					url: 'http://124.222.61.241:6868/api/ESC/App_InventorySubmit_Eam',
					method: 'POST',
					data: {
						roleId: this.userInfo.RoleID,
						userId: this.userInfo.UserID,
