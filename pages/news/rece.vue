			...
			<template v-if="list && list.length > 0">
				<checkbox-group @change="checkboxChange">
					<view v-for="(item, index) in list" :key="index">
						...
						<view :class="['list-box',{'list-box2' :item.status!=0}]">
							<view class="list-head-block">
								<text class="shop-name">检具编号： &emsp13;{{item.assetsCode}}</text>

								<checkbox v-if="item.status==0" style="transform: scale(0.9)" :value="item.ID"
	...
	export default {
		data() {
			return {
				...
				// 	actualType: null,
				// 	actualDep: "CZM 加工中心 Milling Center",
				// 	actualUserId: 22,
				// 	actualUser: "0002",
				// 	actualLocal: null,
				// 	actualState: 0,
				// 	assetsFlag: 0,
				// 	assetsEpc: "F000000007301305",
				// 	assetsImg: "",
				// 	assetsCategory: null,
				// 	assetsTagType: null,
				// 	bindCardStatus: 0,
		...
		onShow() {
			...
			if (epc.length != 0) {
				...
				//添加新数据
				uni.request({
					url: "http://124.222.61.241:6868/api/ESC/App_AssetsQuery_Eam",
					data: {
			...
			console.log("noback2====" + uni.getStorageSync("noReceList2"))
			if (uni.getStorageSync("noReceList2").length > 0) {

				if (uni.getStorageSync("noReceList2") == 'empty') {
					this.list = []
				} else {
					this.list = uni.getStorageSync("noReceList2")
				}

			}

			uni.setStorageSync("noReceList2", '')

		}
		,
		onPullDownRefresh() {
			getApp().globalData.dataInfo.length=0
		...
		methods: {
			...
			checkboxChange(e) {
				...
				this.ids = [];
				this.disabled = true
				this.list.filter(item => item.status == 0).forEach((item) => {
...
,
			...
			next() {
				...
				if (noList[0] == null) {
					uni.setStorageSync("noList", "empty")

				} else {
					uni.setStorageSync("noList", noList)
				}

				uni.setStorageSync("List", this.ids)


				uni.$u.route('/pages/rece/next')
			}
			...
			viewDetail(item) {
				...
				uni.$u.route('/pages/rece/rece-detail?options=' + item)
			},
			/**
			 * 关闭筛选
			 */
