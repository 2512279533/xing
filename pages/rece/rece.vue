					...
					<view :class="['list-box',{'list-box2' :item.status!=0}]"
						...
						<view class="list-head-block">
							<text class="shop-name">检具编号： &emsp13;{{item.assetsCode}}</text>
								...
								<checkbox  v-if="item.status==0" style="transform: scale(0.9)" :value="item.ID" :checked="item.checked" />
		...
		methods: {
			...
			changeBook(e) {
				...
				if (e.detail.value.length == 0) {
					...
					this.list.map(item =>{if(item.status==0){
						this.ids.push(item)
						this.sum=this.sum+1
						 }
					})
